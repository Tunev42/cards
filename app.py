...
class User(db.Model):
    ...
    admin_responses = db.relationship('AdminResponse', backref='admin_ref', lazy=True)
...
class Complaint(db.Model):
    ...
    admin_responses = db.relationship('AdminResponse', backref='complaint_ref', lazy=True, cascade='all, delete-orphan')
    ...
    def has_admin_response(self):
        return len(self.admin_responses) > 0
...
class AdminResponse(db.Model):
    ...
    complaint_id = db.Column(db.Integer, db.ForeignKey('complaint.id'))
    status_after = db.Column(db.Integer, db.ForeignKey('status.id'))

    # Для удобного доступа
    @property
    def admin(self):
        return self.admin_ref

    @property
    def status(self):
        return Status.query.get(self.status_after) if self.status_after else None
...
def complaint_detail(id):
    ...
    comments = Comment.query.filter_by(complaint_id=id, is_admin_response=False).order_by(
        Comment.created_at.desc()).all()

    # Ответы администратора
    admin_responses = AdminResponse.query.filter_by(complaint_id=id).order_by(AdminResponse.created_at.desc()).all()
...
def delete_complaint(id):
    ...
    # Удаляем связанные комментарии и ответы администратора
    Comment.query.filter_by(complaint_id=id).delete()
    AdminResponse.query.filter_by(complaint_id=id).delete()
...
# ==================== ОТВЕТЫ АДМИНИСТРАТОРА ====================
@app.route('/complaint/<int:id>/admin_response', methods=['POST'])
@admin_required
def add_admin_response(id):
    """Добавление ответа администратора на жалобу"""
    complaint = Complaint.query.get_or_404(id)
    ...
    admin_response = AdminResponse(
        ...
        complaint_id=id,
        status_after=status_id
    )

    # Обновляем статус жалобы, если указан новый статус
    if status_id:
        complaint.status_id = status_id

    complaint.updated_at = datetime.utcnow()

    # Также создаем комментарий для отображения в общей ленте
    comment = Comment(
        text=f"📢 Ответ администратора: {text}",
        user_id=session['user_id'],
        complaint_id=id,
    ...
    flash('Ответ администратора успешно добавлен', 'success')
    return redirect(url_for('complaint_detail', id=id))


@app.route('/admin/response/<int:id>/edit', methods=['GET', 'POST'])
@admin_required
def edit_admin_response(id):
    """Редактирование ответа администратора"""
    response = AdminResponse.query.get_or_404(id)

    if request.method == 'POST':
        response.text = request.form.get('text', '').strip()
        status_id = request.form.get('status_id', type=int)

        if status_id:
            response.status_after = status_id
            # Обновляем статус жалобы
            response.complaint_ref.status_id = status_id
            response.complaint_ref.updated_at = datetime.utcnow()

        db.session.commit()
        flash('Ответ администратора обновлен', 'success')
        return redirect(url_for('complaint_detail', id=response.complaint_id))
...
@app.route('/admin/response/<int:id>/delete', methods=['POST'])
@admin_required
def delete_admin_response(id):
    """Удаление ответа администратора"""
    response = AdminResponse.query.get_or_404(id)
    complaint_id = response.complaint_id

    # Удаляем соответствующий комментарий
    comment = Comment.query.filter_by(
        complaint_id=complaint_id,
        is_admin_response=True
    ).filter(Comment.text.contains(response.text[:50])).first()
    ...
    flash('Ответ администратора удален', 'success')
...
def admin_dashboard():
    ...
    stats = {
        ...
        'pending_responses': Complaint.query.filter(
            Complaint.status_id == 1  # Новые жалобы
        ).count()
    }

    recent_complaints = Complaint.query.order_by(Complaint.created_at.desc()).limit(10).all()
    recent_users = User.query.order_by(User.created_at.desc()).limit(10).all()
    complaints_need_response = Complaint.query.filter(
    ...
    return render_template('admin/dashboard.html',
                           ...
                           complaints_need_response=complaints_need_response)
...
def admin_complaints():
    ...
    has_response = request.args.get('has_response', '')
    ...
    if has_response == 'yes':
        # Жалобы с ответами администратора
        query = query.filter(Complaint.admin_responses.any())
    elif has_response == 'no':
        # Жалобы без ответов администратора
        query = query.filter(~Complaint.admin_responses.any())
...
@app.route('/admin/complaint/<int:id>/quick_response', methods=['POST'])
@admin_required
def admin_quick_response(id):
    """Быстрый ответ администратора"""
    complaint = Complaint.query.get_or_404(id)

    action = request.form.get('action', '')
    text = request.form.get('text', '').strip()

    if not text and action != 'delete':
        flash('Введите текст ответа', 'error')
        return redirect(url_for('admin_complaints'))

    if action == 'respond':
        # Добавляем ответ администратора
        admin_response = AdminResponse(
            text=text,
            admin_id=session['user_id'],
            complaint_id=id,
            status_after=complaint.status_id
        )
    ...
    if new_status_id:
        ...
        if response_text:
            admin_response = AdminResponse(
                ...
                admin_id=session['user_id'],
                complaint_id=id,
                status_after=new_status_id
            )

            comment = Comment(
                text=f"📢 Статус изменен: {complaint.status_ref.name}. {response_text}",
                user_id=session['user_id'],
                complaint_id=id,
                is_admin_response=True
            )
